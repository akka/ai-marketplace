...
- J1 [CRITICAL]: All sealed interface subtypes (ESE events, workflow step input variants) have `@TypeName("...")` on each variant record — essential for maintainability and correct routing; the type name is persisted with the JSON payload and used to deserialize events and route them to the appropriate `Consumer` or `View` `TableUpdater` handler. Without it, FQCN is used and renaming or repackaging the class breaks deserialization. `@TypeName` values must be unique per Akka service. Note: plain records (entity state, KVE state, workflow state) do NOT need `@TypeName` — it is only required for sealed interface subtypes where the runtime must distinguish between variants. Ref: serialization docs — type name, event-sourced-entities docs, views docs, ai-coding-assistant-guidelines.
