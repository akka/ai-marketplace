...
handoffs:
  - label: Implement Convergence Tasks
    ...
    prompt: Complete the tasks appended by convergence
    send: true
  - label: Review Implementation
    agent: akka.review
    prompt: Review the implementation now that convergence reports no remaining gaps
    send: true
---
...
Run `akka specify mode --allows converge`. It exits non-zero when the project's mode
...
`/akka:implement` can complete it. This command MUST run only after
`/akka:implement` has run on the current `tasks.md`, and after
`/akka:tasks` has produced a complete `tasks.md`.
...
**Constitution Authority**: The project constitution (loaded via `akka_sdd_constitution`)
is **non-negotiable**. Code that violates a MUST principle is the highest-severity finding
and produces a corresponding remediation task. If the constitution is an unfilled template
or unavailable, skip constitution checks gracefully rather than failing.

## Execution Steps

### 1. Initialize Convergence Context

Call the `akka_sdd_list_specs` MCP tool to find features. Identify the target feature's
FEATURE_DIR and verify that `spec.md`, `plan.md`, and `tasks.md` all exist (has_spec,
has_plan, has_tasks must all be true). Derive absolute paths:

- SPEC = FEATURE_DIR/spec.md
- PLAN = FEATURE_DIR/plan.md
- TASKS = FEATURE_DIR/tasks.md

If any of these is missing, STOP with a clear, actionable message naming the prerequisite
command to run (`/akka:specify` for a missing spec, `/akka:plan` for a
missing plan, `/akka:tasks` for missing tasks). Do not produce partial output.

Load the constitution via `akka_sdd_constitution`. If it returns an unfilled template or
is otherwise unavailable, note this and skip constitution checks in later steps.
...
|----|----------|----------|--------|----------|----------------|
| F1 | missing  | HIGH     | FR-008 | Example: no append-only guard detected in src/main/java/... when writing tasks.md | Add append-only enforcement |
...
- On `tasks_appended`: state how many tasks were appended under which phase, and recommend
  running `/akka:implement` to complete them; note that a follow-up converge
  run will find fewer or no remaining items.
- On `converged`: recommend proceeding to `/akka:review` or opening a PR. No
  further implement pass is needed for this feature's specified scope.

## Done When

- [ ] `akka_sdd_list_specs` returned a valid FEATURE_DIR and `spec.md`, `plan.md`, `tasks.md` were all located (or the command stopped with the correct prerequisite guidance).
- [ ] Constitution was loaded via `akka_sdd_constitution` (or skipped gracefully because it is an unfilled template / unavailable).
- [ ] The intent inventory covers every FR-###, buildable SC-###, user-story acceptance scenario, plan decision, and (if loaded) MUST-level constitution principle in the current artifacts.
- [ ] Every finding traces to an explicit `source-ref` and carries a `gap-type` from {`missing`, `partial`, `contradicts`, `unrequested`}.
- [ ] Constitution MUST violations, if any, are reported as CRITICAL and appear first in the appended task list.
- [ ] Outcome is exactly one of `converged` or `tasks_appended`, reported explicitly.
- [ ] On `converged`: `tasks.md` is byte-for-byte unchanged (no empty `## Phase N: Convergence` header).
- [ ] On `tasks_appended`: a single new `## Phase N: Convergence` section was appended, task IDs continue from `M+1`, no existing task was modified or renumbered, and no application code was created, modified, or deleted.
- [ ] Next-action recommendation names the specific follow-up command (`/akka:implement` or `/akka:review`).
