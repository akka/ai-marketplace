...
handoffs:
  - label: Run Conformance
    agent: akka.specify
    prompt: Run the auditors and report the definition-of-done verdict
    send: true
  - label: Resolve Open Items
    agent: akka.specify
    prompt: Resolve the exit conditions that are waiting on me
    send: true
---
...
Run `akka specify mode --allows status`. Add `--as-engine` when you are carrying
