...
7. Report completion with branch name, spec file path, and checklist results.

   **The next step depends on the mode.** Run `akka specify mode` and read it.

   - **À la carte** — recommend the next command (`/akka:clarify` or
     `/akka:plan`) and stop. The developer drives.
   - **Enforced** — do not name a command. Those steps are refused when a person
...
- [ ] Completion was reported with branch name, spec file path, and checklist results.
- [ ] The mode was read, and the next step matched it: a recommendation in À la
      carte, and the sequence carried out here in Enforced, with no command named
