...
Walk through every check below. For each check, report PASS, WARN
(acceptable deviation with reason), or FAIL (must fix). For WARN and FAIL
items, cite the specific file(s) and line(s).
...
## Akka SDK Best Practices Checklist
...
- K6 [RECOMMENDED]: View row records are public, named `Entry` or `{Domain}Entry`
...
- Use file search, grep, and read tools to gather evidence — do NOT assume compliance without checking code
- Check a representative sample (3-5 files per category) for pattern-based checks
- For one-of-a-kind checks (e.g., "does `definition()` exist"), grep the whole codebase
...
- CRITICAL findings are reported as FAIL; RECOMMENDED findings are reported as WARN
...
K. Architecture & Conventions      | N/6        | RECOMMENDED
...
Report CRITICAL findings first, then RECOMMENDED. For each finding:
```
[ID] FAIL/WARN — Description
...
Rank the 5 most impactful CRITICAL items by: (1) data integrity risk,
(2) runtime failure risk, (3) security risk.
