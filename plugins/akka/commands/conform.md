...
handoffs:
  - label: Fix Blockers
    ...
    send: true
  - label: Ship for Review
    agent: akka.ship
    prompt: Run the auditors for the review target and, on pass, run the review ship steps
    send: true
---
...
Run `akka specify mode --allows conform`. Add `--as-engine` when you are carrying
