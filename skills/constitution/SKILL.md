...
Run `akka specify mode --allows constitution`. It exits non-zero when the project's mode
...
**Note**: If the project constitution does not exist yet, it should have been initialized from `.akka/templates/constitution-template.md` during project setup. If it's missing, copy the template first.

Follow this execution flow:

1. Load the existing constitution at the project constitution (via `akka_sdd_constitution`).
...
4. Consistency propagation checklist (convert prior checklist into active validations):
   - Read `.akka/templates/plan-template.md` and ensure any "Constitution Check" or rules align with updated principles.
   - Read `.akka/templates/spec-template.md` for scope/requirements alignment—update if constitution adds/removes mandatory sections or constraints.
   - Read `.akka/templates/tasks-template.md` and ensure task categorization reflects new or removed principle-driven task types (e.g., observability, versioning, testing discipline).
   - Read each command file in `.akka/templates/commands/*.md` (including this one) to verify no outdated references (agent-specific names like CLAUDE only) remain when generic guidance is required.
...
7. Write the completed constitution back to the project constitution (via `akka_sdd_constitution`) (overwrite).
